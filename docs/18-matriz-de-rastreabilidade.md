...
| `EV-RELSEC-3` | ciclo anterior: audit vazio, Gitleaks sem finding e licença/provenance verdes; o gate hash-bound atual ainda exige nova execução | [`release-security-gates.ts`](../scripts/release-security-gates.ts) |
...
| `BLK-MULTIPLATFORM` | CI/platform owner | bloqueia release | runs, JUnit classificado e artifacts reais dos seis pares OS/arquitetura; skip exige waiver explícito |
...
| R044 | S08 | `validado-localmente` | `EV-PTY-3X` e [`dashboard.test.ts`](../packages/tui/tests/dashboard.test.ts). |
...
| R070 | S11/S12 | `parcial` | `EV-WIN-NATIVE`, `EV-S11-FS-12`, `EV-S12-DIST-8` e `EV-CI-S11-SOURCE`; o workflow cobre seis pares, path longo/Unicode, process tree/signals, keychain fake, Git/sandbox e PTY, mas runs reais dos demais targets permanecem em `BLK-MULTIPLATFORM`. |
